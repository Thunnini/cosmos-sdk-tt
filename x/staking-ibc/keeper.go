...
func (keeper StakingIBCKeeper) Delegate(ctx sdk.Context, from sdk.AccAddress, validatorAddr sdk.ValAddress, amount sdk.Coin, destAddr []byte) sdk.Error {
	store := ctx.KVStore(keeper.storeKey)
	key := from.Bytes()
	// Due to the lack of development time, limit delegators to delegate only once at a time.
	if store.Has(key) {
		return sdk.ErrInternal("Can't delegate twice")
	}
	...
	if !found {
		return sdk.ErrInternal("Unkwon validator")
	}

	_, err = keeper.stakingKeeper.Delegate(ctx, keeper.supplyKeeper.GetModuleAddress(recipientModuleName), amount.Amount, sdk.Unbonded, validator, true)
	...
	err = keeper.ibcKeeper.SendPacket(ctx, "TODO", mock.MockPacket{
		Data: bz,
	})
